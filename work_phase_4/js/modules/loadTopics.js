...
export async function loadTopics(minID) {
    ...
    function createTopicPanels(posts) {
        ...
        posts.forEach((post) => {
            if (Number(post.id) > minID) {
                ...
                let avatarUrl = normalizeAvatarSrc(
                    descriptionParts[0].replace(/[ |]+$/, "")
                );
    ...
    function createCommentElement(comment, index) {
        ...
        const avatarUrl = normalizeAvatarSrc(contentParts[0]);
        const avatarImg = document.createElement("img");
        avatarImg.src = avatarUrl;
    ...
    function normalizeAvatarSrc(avatarSrc) {
        return avatarSrc?.startsWith("http")
